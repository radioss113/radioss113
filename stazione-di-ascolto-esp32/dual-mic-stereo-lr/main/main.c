...
#define MIC_PROFILE_NAME                    "A1S dual raw left/right test"
#define MIC_ES8388_GAIN_DB                  24
#define CAPTURE_FLATLINE_ABS_THRESHOLD       1
#define CAPTURE_FLATLINE_READS_TRIGGER     282
#define CAPTURE_FLATLINE_COOLDOWN_MS     60000
#define STREAM_LED_ACTIVE_HOLD_MS         3000
...
static bool samples_look_flatlined_i16(const int16_t *samples, size_t count, int16_t abs_threshold)
{
    if (!samples || count == 0) {
        return false;
    }
...
static bool stream_led_should_be_on(void)
{
    if (!wifi_station_is_connected()) {
        return false;
    }

    harbor_client_stats_t harbor = {0};
    stream_stats_t stream = {0};
    ...
    if (!harbor.connected || !harbor.accepted) {
        return false;
    }

    if (stream.last_stream_ok_us <= 0) {
        return false;
    }

    return (now_us - stream.last_stream_ok_us) <= ((int64_t)STREAM_LED_ACTIVE_HOLD_MS * 1000LL);
}
...
static void capture_task(void *arg)
{
    ...
    wifi_station_stats_t wifi_stats = {0};
    uint32_t last_disconnect_count = 0;
    uint32_t flatline_consecutive_reads = 0;
    int64_t flatline_recovery_cooldown_until_us = 0;
#if ENABLE_PCM_CONDITIONER
    ...
    while (true) {
        ...
        mic_source_config_t mic_cfg = {
            ...
            .primary_mode = MIC_SOURCE_PRIMARY_STEREO_RAW,
        };
...
#endif
        frame_fill = 0;
        flatline_consecutive_reads = 0;
        flatline_recovery_cooldown_until_us = 0;
        wifi_station_get_stats(&wifi_stats);
        last_disconnect_count = wifi_stats.disconnect_count;
        ...
        while (true) {
            ...
            wifi_station_get_stats(&wifi_stats);
            if (wifi_stats.disconnect_count != last_disconnect_count) {
                last_disconnect_count = wifi_stats.disconnect_count;
                flatline_consecutive_reads = 0;
                ESP_LOGW(TAG,
                         "Wi-Fi disconnect observed, capture flatline watchdog remains active count=%" PRIu32,
                         wifi_stats.disconnect_count);
            }

            int64_t now_us = esp_timer_get_time();
            if (now_us >= flatline_recovery_cooldown_until_us) {
                if (samples_look_flatlined_i16(read_block, (size_t)samples, CAPTURE_FLATLINE_ABS_THRESHOLD)) {
                    flatline_consecutive_reads++;
                    if (flatline_consecutive_reads >= CAPTURE_FLATLINE_READS_TRIGGER) {
                        capture_stats_note_failure();
                        flatline_recovery_cooldown_until_us = now_us + ((int64_t)CAPTURE_FLATLINE_COOLDOWN_MS * 1000LL);
                        ESP_LOGW(TAG,
                                 "Capture flatline detected: restarting mic path"
                                 " (reads=%" PRIu32 ", disconnects=%" PRIu32 ", cooldown_ms=%d)",
                                 flatline_consecutive_reads,
                                 wifi_stats.disconnect_count,
                                 CAPTURE_FLATLINE_COOLDOWN_MS);
                        flatline_consecutive_reads = 0;
...
void app_main(void)
{
    ...
    const char *device_name = s_runtime_config.stream_device_name[0] ? s_runtime_config.stream_device_name : "soundscape";
    ...
    while (true) {
        ...
        wifi_station_set_streaming_active(stream_led_should_be_on());
