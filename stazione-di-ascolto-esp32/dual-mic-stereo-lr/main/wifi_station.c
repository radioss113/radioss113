...
#define AP_SSID               "Radio SS113 Alfredo"
...
#define STATUS_LED_SEARCH_PERIOD_MS 1000
#define STATUS_LED_SLOW_PERIOD_MS 2000
...
static void refresh_status_led_mode(void)
{
    if (s_status_led_mode == STATUS_LED_FACTORY_RESET) {
        return;
    }
    if (s_streaming_active) {
        status_led_set_mode(STATUS_LED_ON);
    } else if (!s_runtime_config.have_wifi_credentials) {
        status_led_set_mode(STATUS_LED_SEARCH);
    } else {
        status_led_set_mode(STATUS_LED_SLOW_BLINK);
    }
}
    ...
    while (true) {
        ...
        switch (mode) {
            case STATUS_LED_ON:
                ...
                continue;
            case STATUS_LED_SEARCH:
                period_ms = STATUS_LED_SEARCH_PERIOD_MS;
                break;
...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    ...
    html_escape(s_stats.connected ? s_runtime_config.saved_pass : "", saved_pass_html, sizeof(saved_pass_html));
    html_escape(s_runtime_config.stream_uri[0] ? s_runtime_config.stream_uri : CONFIG_HARBOR_URI,
                stream_base_html,
                sizeof(stream_base_html));
    html_escape(s_runtime_config.stream_device_name[0] ? s_runtime_config.stream_device_name : "test",
                stream_name_html,
                sizeof(stream_name_html));
    html_escape(active_stream_password(&s_runtime_config), stream_password_html, sizeof(stream_password_html));
    ...
    html_appendf(html, html_cap, &off,
                 ...
                 "<label>Password<br><input id='password' name='password' type='password' maxlength='64' value='%s'></label><br><br>"
...
void wifi_station_set_streaming_active(bool active)
{
